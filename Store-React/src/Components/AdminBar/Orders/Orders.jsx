...
export default function Orders() {
  const [Currentorders, setCurrentOrders] = useState([]); // تخزين الطلبات المعروضة
  ...
  const FindOrder = async (OrderId) => {
    if (!OrderId) {
      setSearchMode(false);
      setCurrentOrders(allOrders); // استرجاع الطلبات الأصلية عند مسح البحث
      return;
    }
    ...
    try {
      ...
      setCurrentOrders([data]); // عرض الطلب المحدد فقط
      setSearchMode(true);
    } catch (err) {
